...
export async function getSalesSummary(db: Database, range: DateRange): Promise<SalesSummary> {
  const normalized = normalizeRange(range);
  const sales = (await listSales(db)).filter((s) => inRange(s.createdAt, normalized));
...
export async function getTopProducts(db: Database, range: DateRange, limit = 10): Promise<TopProduct[]> {
  const normalized = normalizeRange(range);
  ...
  const saleIdsInRange = new Set(sales.filter((s) => inRange(s.createdAt, normalized)).map((s) => s.id));
...
export async function getMarginsSummary(db: Database, range: DateRange): Promise<MarginsSummary> {
  const normalized = normalizeRange(range);
  const [sales, allSaleItems, variants, products] = await Promise.all([
    listSales(db),
    db.select().from(schema.saleItems),
  ...
  const salesInRange = new Map(
    sales.filter((s) => inRange(s.createdAt, normalized)).map((s) => [s.id, s] as const),
  );
...
export async function getCashFlow(db: Database, range: DateRange): Promise<CashFlow> {
  const normalized = normalizeRange(range);
  const [payments, creditRepayments, debtPayments] = await Promise.all([
  ...
  for (const r of creditRepayments) bump(r.paidAt, r.amount, 0);
  for (const d of debtPayments) bump(d.paidAt, 0, d.amount);
...
export async function projectCashFlow(
  ...
  const { byMonth } = await getCashFlow(db, {
